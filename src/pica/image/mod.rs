...
mod test {
    ...
    use crate::pica::image::get;

    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    pub async fn test_preview_arw() -> Result<()> {
        tracing_subscriber::fmt::init();

        let path = get("_test/DSC05254.ARW").await?;
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    pub async fn test_preview_cr3() -> Result<()> {
        tracing_subscriber::fmt::init();

        let path = get("_test/CR6_1519 (1).CR3").await?;
        std::fs::copy(&path, "/tmp/preview.jpg")?;
