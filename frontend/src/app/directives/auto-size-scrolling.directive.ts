...
import {CdkVirtualScrollViewport, VIRTUAL_SCROLL_STRATEGY, type VirtualScrollStrategy} from '@angular/cdk/scrolling';
import {Directive, forwardRef, Input, numberAttribute, type OnChanges} from '@angular/core';
...
class ItemSizeAverager {
  ...
  private readonly _defaultItemSize: number;
...
class AutoSizeVirtualScrollStrategy implements VirtualScrollStrategy {
  ...
  scrolledIndexChange = new Observable<number>(() => {
    ...
    if (typeof anyWindow.ngDevMode === 'undefined' || anyWindow.ngDevMode) {
      throw Error(
...
@Directive({
  standalone: true,
  selector: 'cdk-virtual-scroll-viewport[my-autosize]',
  providers: [
    {
      provide: VIRTUAL_SCROLL_STRATEGY,
      useFactory: _myAutoSizeVirtualScrollStrategyFactory,
      deps: [forwardRef(() => MyAutoSizeVirtualScroll)],
    },
  ],
})
export class MyAutoSizeVirtualScroll implements OnChanges {
  /**
   * The minimum amount of buffer rendered beyond the viewport (in pixels).
  ...
  readonly _scrollStrategy = new AutoSizeVirtualScrollStrategy(this.minBufferPx, this.maxBufferPx);
 ...
 */
export function _myAutoSizeVirtualScrollStrategyFactory(autoSizeDir: MyAutoSizeVirtualScroll): AutoSizeVirtualScrollStrategy {
