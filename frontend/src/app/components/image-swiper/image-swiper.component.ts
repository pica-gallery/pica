...
import {Image, ImageId} from '../../service/api';
...
export class ImageSwiperComponent implements AfterViewInit, OnDestroy {
    ...
    @Input()
    public imageToShow: ImageId | null = null;
    ...
    protected readonly trackByImageId = (_idx: number, image: Image) => image.id;
    ...
    ngAfterViewInit(): void {
        ...
        let currentIdx = Math.max(0, this.images.findIndex(img => img.id === this.imageToShow));

        // preload the images
        this.imageViews.get(currentIdx)?.load();
        this.imageViews.get(currentIdx - 1)?.load();
        this.imageViews.get(currentIdx + 1)?.load();
        ...
        this.ngZone.runOutsideAngular(() => {
            ...
            this.container.nativeElement.addEventListener('touchstart', event => {
                ...
                if (event.touches.length === 2) {
                    ...
                    p1 = {x: event.touches[0].screenX, y: event.touches[0].screenY};
                    p2 = {x: event.touches[1].screenX, y: event.touches[1].screenY};
                }
            ...
            this.container.nativeElement.addEventListener('touchmove', event => {
                const curX = event.touches[0].screenX;
                ...
                if (p1 && p2) {
                    ...
                    const cp1 = {x: event.touches[0].screenX, y: event.touches[0].screenY};
                    const cp2 = {x: event.touches[1].screenX, y: event.touches[1].screenY};
